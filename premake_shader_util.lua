...
local function compile_shaders(EXECUTABLE, BASE_ARGS, VK_BASE_ARGS, PERMUTATION_ARGS, INCLUDES_ARGS, SHADER_FILES, PERMUTATION_OUTPUTS)
	local result = {}
	...
	for _, PASS_SHADER in ipairs(SHADER_FILES) do
		...
		local wave32_command = string.format('%s %s -name="%s" -DFFX_HALF=0 %s -output="%s" "%s"', 
											 executable_call, sc_args, PASS_SHADER_FILENAME, INCLUDES_ARGS, PERMUTATION_OUTPUTS, PASS_SHADER)
